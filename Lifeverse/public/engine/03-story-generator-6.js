...
/* ------------------------------------------------------------------
 * 메모 반영 (v4.1: 방식1 인용 + 방식2 상태번역 하이브리드)
 * ------------------------------------------------------------------
 * 사용자가 남긴 현실 메모("무릎이 아팠지만 끝까지 뛰었다")를 이야기에
 * 녹인다. 두 층위로 작동한다:
 *   방식1 (항상): 메모를 이야기 마지막에 자연스러운 틀로 인용한다.
 *   방식2 (키워드가 명확할 때만): 메모에서 신체부위/상태를 추출해,
 *     본문 도입부에 세계관 톤의 한 문장으로 얹는다. "오늘 레오의 무릎은
 *     성치 않았다" 처럼. 애매하면 방식2는 생략하고 방식1만 쓴다(오탐 방지).
 *
 * 부분 문자열 오탐("목표"의 '목')을 막기 위해 신체부위는 조사가 붙은
 * 형태로만 매칭한다. 상태 키워드도 광범위한 것('좋았')은 넣지 않는다.
 */
var MEMO_BODY_PARTS = ['무릎', '허리', '어깨', '손목', '발목', '종아리', '허벅지', '다리', '손', '발', '목', '눈'];
var MEMO_STATES = [
  { kw: ['힘들', '버거', '벅찼', '벅차'], hard: true },
  { kw: ['아프', '아팠', '아픔', '쑤시', '결려', '결렸', '저리', '뻐근'], hard: true, body: true },
  { kw: ['졸리', '졸려', '못 잤', '못잤', '피곤', '지쳤', '지쳐', '지침'], hard: true },
  { kw: ['개운', '가뿐', '상쾌', '뿌듯', '보람', '후련'], hard: false },
  { kw: ['귀찮', '하기 싫', '하기싫', '내키지', '의욕이 없', '의욕 없'], hard: true },
];

function analyzeMemo(memo) {
  const m = memo || '';
  let bodyPart = null;
  for (const b of MEMO_BODY_PARTS) {
    if (new RegExp(b + '(이|가|을|를|에|은|는|도|만|까지)').test(m)) { bodyPart = b; break; }
  }
  let hard = null; // true=힘든 톤, false=좋은 톤, null=불명
  for (const s of MEMO_STATES) {
    if (s.kw.some((k) => m.includes(k))) { hard = s.hard; break; }
  }
  return { bodyPart, hard };
}

/**
 * 방식2: 메모의 상태를 세계관 톤 한 문장으로. 명확한 신호가 없으면 null.
 * 이 문장은 본문 도입부(opener 앞)에 얹힌다.
 */
function buildMemoStateLine(memo, heroName) {
  const { bodyPart, hard } = analyzeMemo(memo);
  const H = heroBase(heroName);
  if (bodyPart) {
    // 신체부위가 잡히면 그 부위를 구체적으로 언급 (가장 몰입도 높음)
    // 조사 은/는: heroTopic과 같은 규칙을 부위 단어에 적용
    const partTopic = (function (w) {
      const last = w.charCodeAt(w.length - 1);
      if (last < 0xac00 || last > 0xd7a3) return w + '는';
      return w + (((last - 0xac00) % 28 !== 0) ? '은' : '는');
    })(bodyPart);
    if (hard === false) return '오늘 ' + heroPoss(heroName) + ' ' + partTopic + ' 어느 때보다 가뿐했다.';
    return '오늘 ' + heroPoss(heroName) + ' ' + partTopic + ' 성치 않았다. 그럼에도 ' + H + '는 걸음을 멈추지 않았다.';
  }
  if (hard === true) {
    const HARD = [
      '오늘의 길은 유난히 무겁게 느껴졌다.',
    ...
    return HARD[Math.abs(hashStr((memo || '') + 'hs')) % HARD.length];
  }
  if (hard === false) {
    const GOOD = [
      '오늘은 몸도 마음도 가벼웠다.',
      '발걸음이 어느 때보다 가뿐한 날이었다.',
      '어쩐지 오늘은 모든 것이 순하게 흘렀다.',
    ];
    return GOOD[Math.abs(hashStr((memo || '') + 'gd')) % GOOD.length];
  }
  return null; // 불명 — 방식2 생략
}

/** 방식1: 메모를 이야기 끝에 인용하는 틀 (v3 유지, 문구만 다듬음) */
...
function generateTodayStory(input) {
  ...
  // 방식2: 메모에서 명확한 상태가 잡히면, 본문 도입부에 세계관 톤 한 문장을
  // 얹어 core와 이어 붙인다("오늘 레오의 무릎은 성치 않았다. [opener]…").
  // 잡히지 않으면(null) core를 그대로 쓴다. 방식1(끝 인용)은 아래에서 항상 실행.
  const memoStateLine = trimmedMemo ? buildMemoStateLine(trimmedMemo, heroName) : null;
  const coreWithState = memoStateLine ? (memoStateLine + ' ' + core) : core;

  const paragraphs = [];
  if (revealText) paragraphs.push(revealText);
  paragraphs.push(coreWithState);
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    ...
    buildRestDayLine, buildActScene, buildMemoLine, buildMemoStateLine, analyzeMemo, buildOpenerCloser, assembleCore,
