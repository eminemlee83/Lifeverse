/**
 * LIFEVERSE Story Engine — Layer 2: Narrative Rule Engine (v3)
 * ============================================================
 ...
 *   stageTransition 세계관 단계가 바뀌는 날 (8일째, 15일째, v3부터 22·29일째 포함)
 *   halfway         여정의 정확한 중간 날 (10일 이상 목표만)
 *   finalStretch    남은 날 1~2일
 *   ordinary        평범한 날 (기존)
 *
 * v3 추가 (비파괴적 확장 — 분기는 늘리지 않는다):
 *   반환값에 progressRatio(0~1)와 act(1|2|3|null)를 추가했다.
 *   act는 3막 구조 — 1막(도입, ~30%), 2막(전개, ~75%), 3막(절정, 그 이후).
 *   Layer 3이 평범한 날의 문장 톤을 진행도에 따라 미세 조정하는 데 쓴다.
 *   ⚠️ act는 "분기"가 아니다 — 매일 바뀌지 않는 구간 정보일 뿐이며,
 *   resolveBranch는 건드리지 않았다 (v11 교훈: 구간 전체를 특별하게 만들면
 *   특별함이 죽는다. act는 특별함이 아니라 배경 톤에만 쓸 것).
 *   targetDateIso가 없으면 progressRatio/act는 null이다 (하위호환).
 *
...
/** 3막 구조 경계 (v3). 1막: ~30%, 2막: ~75%, 3막: 그 이후. */
const ACT_ONE_MAX_RATIO = 0.3;
const ACT_TWO_MAX_RATIO = 0.75;
...
function resolveNarrativeState(input) {
  ...
  // --- 3막 구조 (v3, targetDateIso가 있을 때만) ---
  const progressRatio = daysTotal ? dayNo / daysTotal : null;
  let act = null;
  if (progressRatio !== null) {
    if (progressRatio <= ACT_ONE_MAX_RATIO) act = 1;
    else if (progressRatio <= ACT_TWO_MAX_RATIO) act = 2;
    else act = 3;
  }
  ...
  // 마지막 단계 진입일까지만). 단계가 더 없으면 발동 안 함.
