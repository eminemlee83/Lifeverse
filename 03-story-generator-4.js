...
/**
 * 오늘의 행동을 "middle" 문장으로 만든다 (v3.1 전면 개편).
 *
 * 이전(v3)까지는 장면 라벨("채소 들어간 식사 한 끼 챙기기")을 문장에 그대로
 * 끼워 넣어서, 판타지 서사 한복판에 앱 UI 문구가 튀어나오는 문제가 있었다.
 * 이제는 카테고리의 "행동의 결"(actEssence)로 은은하게 번역한다 —
 * 무엇을 몇 개 했는지는 장면 체크리스트 화면에 이미 있으므로, 이야기 본문은
 * "오늘 무언가를 해서 세계가 조금 움직였다"는 결만 전한다.
 *
 * 행동 가짓수(questCount)는 문장의 '무게'로만 반영한다: 1개면 담담하게,
 * 여러 개면 "여럿을 해내며"처럼 밀도를 살짝 얹는다. 라벨 자체는 노출하지 않는다.
 *
 * @param {Array<string>} questLabels - 오늘 완료한 장면 라벨들 (개수만 사용)
 * @param {string} dateIso
 * @param {string} categoryId
 * @param {number} [dayNo=1] - essence 선택 시드 다양화용
 */
function buildMiddleClause(questLabels, dateIso, categoryId, dayNo) {
  const count = questLabels ? questLabels.length : 0;
  const essenceSeed = seedFor(dateIso || '', categoryId || '', 'essence') + (dayNo || 1);
  const essence = getActEssence(categoryId, essenceSeed); // "몸을 돌보는 일에 마음을 썼다" 등

  // 행동 가짓수를 문장 밀도로만 반영 (라벨 비노출)
  if (count >= 3) {
    const PREFIX = ['여러 가지를 손에서 놓지 않으며, ', '오늘 몫을 하나하나 채워, ', '해야 할 일들을 차례로 지나며, '];
    return PREFIX[seedFor(dateIso || '', categoryId || '', 'mid3') % PREFIX.length] + essence;
  }
  if (count === 2) {
    const PREFIX = ['두 가지를 나란히 해내며, ', '하나에 이어 또 하나를 놓으며, '];
    return PREFIX[seedFor(dateIso || '', categoryId || '', 'mid2') % PREFIX.length] + essence;
  }
  return essence;
}
...
/**
 * 본문 코어(opener + middle + 곁들임)를 부드럽게 잇는다 (v3.1 신규).
 *
 * 이전에는 `opener + ' ' + middle + '.'` 처럼 완결 문장 두 개를 공백으로만
 * 이어 붙여서 "…걸음을 옮겼다. 채소 들어간 식사 한 끼 챙기기를 해냈다."처럼
 * 툭툭 끊겼다. 이제 opener와 middle 사이에 연결 부사구(BRIDGE)를 넣어
 * 하나의 흐름으로 만든다. middle(essence)은 "~다"로 끝나는 서술절이므로
 * BRIDGE(부사구) + middle = 완결된 한 문장이 된다.
 *
 * 곁들임(extraLine: 감각·앰비언트·동행 인물)은 그 성격에 따라 붙인다:
 * 감각/앰비언트는 본문 문장 뒤에 자연스럽게 이어 붙이고, 동행 인물처럼
 * 긴 서사 문장은 새 문장으로 띄운다(이미 마침표로 끝나므로 공백 연결).
 */
function assembleCore({ opener, middle, extraLine, dateIso, categoryId }) {
  // opener를 여는 그날의 흐름을 middle로 잇는 연결 부사구.
  // 전부 쉼표로 끝나서 뒤에 "~다" 서술절이 자연스럽게 붙는다.
  const BRIDGES = [
    '그렇게 오늘도, ',
    '그 걸음 속에서, ',
    '그리고 그 자리에서, ',
  ...
  // opener(마침표로 끝남) + [줄바꿈 없는 자연 연결] + bridge + middle(마침표로 끝남)
  let core = opener + ' ' + bridge + middle + '.';
...
function generateTodayStory(input) {
  ...
  // --- 5) 조립 ---
  const middle = buildMiddleClause(questLabels, dateIso, categoryId, narrativeState.dayNo);
  ...
  // v3.1: 부드러운 연결 — opener와 middle을 마침표로 툭 끊지 않고,
  // 문장을 잇는 연결 어구(bridge)로 하나의 흐름처럼 만든다. opener가
  // 이미 마침표로 끝나는 완결 문장이므로, bridge는 새 문장을 자연스럽게
  // 여는 부사구로 쓴다("그렇게," "그 안에서," 등). middle(essence)이
  // "~다"로 끝나므로 bridge + middle 은 완결된 한 문장이 된다.
  const core = assembleCore({ opener, middle, extraLine, dateIso, categoryId });

  // 다문단 구조 — [회수] / [본문(부드럽게 연결)] / [맺음(일차)]
  const paragraphs = [];
